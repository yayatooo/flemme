...
async function favorite(uid: string, cookingSessionId: string) {
	const response = await request(uid, "POST", "/api/favorites", {
		cookingSessionId,
	});
...
async function list(uid: string, query = "") {
	const response = await request(uid, "GET", `/api/favorites${query}`);
	return {
		response,
		payload:
			response.status === 200
...
test("empty, create, rich projection, delete, and preserved completed History", async () => {
	...
	expect(
		(
			await request(uid, "POST", "/api/favorites", {
				cookingSessionId: cooked.id,
			})
		).status,
	).toBe(409);
	...
	const historyBefore = CookingHistoryPageSchema.parse(
		await (
			await app.request("/api/cooking-sessions/history", {
				headers: headers(uid),
			})
		).json(),
	);
	...
	expect(
		(await request(uid, "DELETE", `/api/favorites/${item.id}`)).status,
	).toBe(204);
...
test("ownership, missing resources, session auth and strict payloads", async () => {
	...
	expect(
		(
			await request(other, "POST", "/api/favorites", {
				cookingSessionId: cooked.id,
			})
		).status,
	).toBe(403);
	...
	expect(
		(await request(other, "DELETE", `/api/favorites/${item.id}`)).status,
	).toBe(403);
	expect(
		(await request(uid, "DELETE", `/api/favorites/${crypto.randomUUID()}`))
			.status,
	).toBe(404);
	expect(
		(
	...
	for (const body of [
		...
		expect((await request(uid, "POST", "/api/favorites", body)).status).toBe(
			400,
		);
	}
});
